...
def init_table(
    path, iteration_name, iteration_unit, extra_columns="", extra_column_units=""
):
...
def fake_min(config, path):
    ...
    if config["save_steps"]:
        ...
        for i in range(1, 10):
            ...
            m_new = df.Field(
                m.mesh, nvdim=3, value=m_new_array.reshape((*m.mesh.n, 3)), norm=m.norm
            )
...
def fake_llg(config, path):
    ...
    for i in range(1, n + 1):
        ...
        damping_rot_field = df.Field(
            m.mesh, nvdim=3, value=H.cross(m).array, norm=damping_angle
        )
        ...
        precession_rot_field = df.Field(
            m.mesh, nvdim=3, value=H.array, norm=precession_angle
        )
        ...
        update_table(
            path, i * dt, -1e-10 * i, m.orientation, alpha_factor, precession_angle
        )
        print(f"Step {i} finished")
